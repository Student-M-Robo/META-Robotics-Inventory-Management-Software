...
# New Import:
from edit_part import EditPartWindow # <--- ADD THIS LINE
...
class InventoryManagementWindow:
    ...
    def open_window(self):
        ...
        buttons = [
            ...
            # --- NEW COMMAND ---
            ("Edit Part Information", self.open_edit_part_information), # <--- CALLS THE NEW METHOD
            # -------------------
        ...
        # Back Page button (Return to Main Menu)
        back_btn = Button(nav_frame, text="Back Page", command=self.return_to_main_menu,
                          bg="#cccccc", font=("Arial", 10))
        back_btn.pack(side="right", padx=10)
        
        # MENU button (Return to Main Menu - redundant but included for consistency with slides)
        ...
        if file_path:
            ...
            try:
                ...
                self.photo_preview_label.config(
                    ...
                    compound=tk.LEFT, 
                    padx=5, 
                    width=180 # Fixed width for layout
                )
                ...
                self.preview_image_ref = None
                self.selected_photo_path = None # Clear path on error
